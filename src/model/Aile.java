...
public class Aile implements InterfaceCorps {
	 ...
	 * @param b
	 *            boolean droite ou gauche
	 */
	public Aile(Oiseau o, boolean b) {
		this.oiseau = o;
		// positions communes a plusieurs points sur un axe
		float ctop = 1.5f * coef, cbot = -1.5f * coef, cfront = 5.0f * coef, crear = 0.0f * coef, cbeg = -2.5f * coef,
				mtop = 1.0f * coef, mbot = -1.0f * coef, mfront = 4.0f * coef, mrear = -1.0f * coef,
				mbeg = -5.0f * coef, ex = -20.0f * coef, ey = 0.0f * coef, ez = -10.0f * coef;
		...
		//Creation des points a partir de leur positions communes
		closeFT = new Point3D(cbeg, ctop, cfront);
		closeRT = new Point3D(cbeg, ctop, crear);

		closeFB = new Point3D(cbeg, cbot, cfront);
		closeRB = new Point3D(cbeg, cbot, crear);

		midFT = new Point3D(mbeg, mtop, mfront);
		midRT = new Point3D(mbeg, mtop, mrear);

		midFB = new Point3D(mbeg, mbot, mfront);
		midRB = new Point3D(mbeg, mbot, mrear);
		
		end = new Point3D(ex, ey, ez);

		//comportement different pour les ailes droites et gauches
		if (b) {
			// top
	 ...
	 * 
	 * @param angle
	 *            float la modification a apporter aux ailes
	 */
